...
def load_data():
        ...
        for team, team_data in teams.items():
            ...
            for pot, history_list in team_data.get("pots", {}).items():
                for entry in history_list:
                    ...
                    is_active = (not start or start <= date_str) and (
                        not end or end > date_str
                    )
    ...
    for team, team_data in teams.items():
        for pot, history_list in team_data.get("pots", {}).items():
            for entry in history_list:
                ...
                is_active = (not start or start <= compare_date) and (
                    not end or end > compare_date
                )
                if is_active:
    ...
    return (
        ...
        team_points_unweighted_cumulative,
        chart_data,
        latest_date,
    )
...
if not chart_data:
    st.warning(
        "No data found. Please run the collector script (`python src/collector.py`)"
    )
    st.stop()

st.write(f"Latest data from: **{latest_date}**")
st.caption(
    "Source: [snooker.org Provisional Season Points](https://www.snooker.org/res/index.asp?template=34) — *Note: This tracker considers guaranteed money for current and upcoming events. Points from Champions League are excluded.*"
)
